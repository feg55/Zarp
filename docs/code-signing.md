...
Workflow поддерживает SignPath, но **по умолчанию публикует неподписанный релиз**. Отсутствие сертификата и секретов не мешает сборке. В описании релиза явно указано, подписан ли EXE. Сборки веток и pull request остаются неподписанными тестовыми артефактами.

Подписание включается переменной репозитория `SIGNPATH_ENABLED=true`. После включения ошибки настройки или проверки подписи останавливают релиз: автоматического перехода к неподписанному EXE при сбое сервиса нет.
...
4. В GitHub → Settings → Secrets and variables → Actions добавьте:
   ...
   | Variable | `SIGNPATH_ENABLED` | `true` — включить подписание после настройки остальных значений |
...
6. После включения `SIGNPATH_ENABLED=true` создайте новый тег версии. Workflow соберёт EXE, отправит артефакт на подпись, проверит доверие Windows и timestamp, затем рассчитает SHA-256, создаст attestation и опубликует релиз. При ошибке включённой подписи неподписанный EXE в Releases не попадёт. Пока сервис не подключён, оставьте `SIGNPATH_ENABLED` отсутствующей или равной `false` — релизы продолжат публиковаться без подписи.
